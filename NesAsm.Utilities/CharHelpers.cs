...
namespace NesAsm.Utilities
{
    public static class CharHelpers
    {
        ...
        public static CharData? LoadImage(string imageFilename, bool hasTileSeparator = true, NesColorPalette[] backgroundPalettes = null, NesColorPalette[] spritePalettes = null)
        {
            ...
            var allPalettes = spritePalettes.Concat(backgroundPalettes).ToArray();

            var spriteTiles = GetTilesAndPalettes(image, 0, hasTileSeparator, allPalettes);
            var backgroundTiles = GetTilesAndPalettes(image, 256, hasTileSeparator, allPalettes);
        ...
        private static IEnumerable<TileData> GetTilesAndPalettes(Png image, int startingTileIndex, bool hasTileSeparator, NesColorPalette[] palettes)
        {
            ...
            for (int tileIndex = 0; tileIndex < 16 * 16; tileIndex++)
            {
                ...
                if (tiles[tileIndex].Palette != null)
                    lastTile = tileIndex;
            }

            return tiles.Take(lastTile + 1);
        }

        private static TileData CreateTileData(Png image, int tileIndex, NesColorPalette[] colorPalettes, bool hasTileSeparator)
        {
            ...
            // First get the palette from the 8x8 tile
            var pixels = new List<byte>();
            var distinctNesColors = new HashSet<byte>();
            for (var j = 0; j < 8; j++)
                for (var i = 0; i < 8; i++)
                {
                    var color = image.GetPixel(tileStartX + i, tileStartY + j);
                    var nesColor = NesColorPalette.MatchNesColor(color, tileStartX + i, tileStartY + j);
                    distinctNesColors.Add(nesColor);
                    pixels.Add(nesColor);
                }

            var colors = distinctNesColors.ToArray();
            ...
            var colorPalette = colorPalettes.FindFromNesColors(colors);

            if (colorPalette == null && colors.Length != 1)
            {
                throw new Exception($"No palette found for tile {tileIndex}, but some colors were found {colors.ToHexDisplay()}");
            }

            // Convert nes color to color index
            for (var i = 0; i < 64; i++)
            {
                pixels[i] = colorPalette.GetColorIndex(pixels[i]);
            }
        ...
        {
            public TileData(byte[] pixels, NesColorPalette palette, int index)
            {
                if (pixels.Length != 64)
            ...
            public NesColorPalette Palette { get; }
        ...
        //    public string NesColorHex => string.Join(" ", NesColors.Select(c => $"{c:X}"));
